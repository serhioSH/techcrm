...
class CobroService(ServicioProtegido):
    ...
    def cobrar(self, venta_id: int, metodo_pago: str,
        ...
        # Registrar efectivo_pago y transferencia_pago correctamente
        efectivo_pago = total if metodo_pago == "EFECTIVO" else 0
        transferencia_pago = total if metodo_pago == "TRANSFERENCIA" else 0
        
        with self._db.transaccion():
            cursor = self._db.execute(
                """UPDATE ventas
                      SET estado='CERRADA', metodo_pago=?, efectivo_pago=?, transferencia_pago=?,
                          cobro_procesado=1, caja_id=?, fecha_hora=datetime('now','localtime')
                    WHERE id=? AND estado='ABIERTA' AND cobro_procesado=0;""",
                (metodo_pago, efectivo_pago, transferencia_pago, caja["id"], venta_id),
            )
    ...
    def cobrar_mixto(self, venta_id: int, efectivo: float, transferencia: float,
        ...
        # Registrar como MIXTO con desglose de efectivo y transferencia
        with self._db.transaccion():
            cursor = self._db.execute(
                """UPDATE ventas
                      SET estado='CERRADA', metodo_pago='MIXTO', efectivo_pago=?, transferencia_pago=?,
                          cobro_procesado=1, caja_id=?, fecha_hora=datetime('now','localtime')
                    WHERE id=? AND estado='ABIERTA' AND cobro_procesado=0;""",
                (round(efectivo, 2), round(transferencia, 2), caja["id"], venta_id),
            )
            if cursor.rowcount != 1:
                raise ValueError("Esta venta ya fue cobrada.")

        # Actualizar totales de caja automáticamente
        self._cajas.actualizar_totales(caja["id"])
