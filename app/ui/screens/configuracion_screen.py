...
class ConfiguracionScreen(BaseScreen):
    ...
    def refrescar(self):
        ...
        # Solo cargar campos de identidad si existen (solo para TECHCRM)
        if hasattr(self, '_txt_nombre'):
            self._txt_nombre.setText(self._config.get("nombre_negocio", ""))
        if hasattr(self, '_txt_direccion'):
            self._txt_direccion.setText(self._config.get("direccion", ""))
        if hasattr(self, '_txt_telefono'):
            self._txt_telefono.setText(self._config.get("telefono", ""))
        if hasattr(self, '_txt_mensaje'):
    ...
    def _aplicar_mesas(self):
        """Aplica cambios en la cantidad de mesas (preserva Domicilios) y guarda en config."""
        nueva_cantidad = int(self._spin_mesas.currentText())
        
        from app.services.mesa_service import MesaService
        from app.services.configuracion_service import ConfiguracionService
        mesa_service = MesaService(self._db, self._auth)
        config_service = ConfiguracionService(self._db, self._auth)
        
        try:
            mesas_actuales = mesa_service.listar()
            
            # Separar mesas normales de la mesa de domicilios
            mesas_normales = [m for m in mesas_actuales if m.get("tipo", "MESA") == "MESA" and m.get("activo", 1) == 1]
            mesa_domicilios = next((m for m in mesas_actuales if m.get("tipo") == "DOMICILIOS"), None)
            
            cantidad_actual = len(mesas_normales)
            
            if nueva_cantidad == cantidad_actual:
                QMessageBox.information(self, "Mesas", f"Ya hay {cantidad_actual} mesas normales.")
                return
            ...
            if nueva_cantidad > cantidad_actual:
                ...
                QMessageBox.information(self, "Mesas", f"+{diferencia} mesas agregadas")
            else:
                # Eliminar mesas normales (desde las más recientes, sin tocar Domicilios)
                diferencia = cantidad_actual - nueva_cantidad
                mesas_ordenadas = sorted(mesas_normales, key=lambda m: m["id"], reverse=True)
                
                for i in range(diferencia):
                    mesa_id = mesas_ordenadas[i]["id"]
                    # Verificar que no sea la mesa de domicilios
                    if mesa_domicilios and mesa_id == mesa_domicilios.get("id"):
                        QMessageBox.warning(self, "Error", "No se puede eliminar la mesa de Domicilios")
                        return
                    self._db.execute("DELETE FROM mesas WHERE id=?;", (mesa_id,))
                
                self._db.commit()
                QMessageBox.information(self, "Mesas", f"-{diferencia} mesas eliminadas")
            
            # GUARDAR LA CANTIDAD EN CONFIGURACIÓN (persistencia)
            try:
                config_service.guardar("cantidad_mesas", str(nueva_cantidad))
            except:
                pass  # Si falla guardar config, al menos las mesas se modificaron
            
            self.refrescar()
        except Exception as e:
            QMessageBox.warning(self, "Error", f"Error al aplicar cambios: {e}")
    ...
    def guardar(self):
        ...
        try:
            ...
            # Solo guardar campos de identidad si existen (TECHCRM)
            if hasattr(self, '_txt_nombre'):
                config_dict["nombre_negocio"] = self._txt_nombre.text().strip()
            if hasattr(self, '_txt_direccion'):
                config_dict["direccion"] = self._txt_direccion.text().strip()
            if hasattr(self, '_txt_telefono'):
                config_dict["telefono"] = self._txt_telefono.text().strip()
            if hasattr(self, '_txt_mensaje'):
                config_dict["mensaje_final"] = self._txt_mensaje.text().strip()
            
            self._config.set_varios(config_dict)
    ...
    def probar_impresion(self):
        ...
        try:
            pm = PrinterManager()
            ok = pm.prueba_impresion(impresora)
            return (True, f"✓ Impresión exitosa") if ok else (False, f"✗ Fallo en la impresión")
        except Exception as e:
            return False, f"✗ Error: {e}"
