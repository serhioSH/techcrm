...
class CajaService(ServicioProtegido):
    ...
    def cerrar(self, caja_id: int, efectivo_contado: float, observaciones: str = "") -> dict:
        """Calcula diferencias, guarda snapshot de inventario, y cierra la caja."""
        self._requerir(Permisos.ABRIR_CERRAR_CAJA)
        ...
        # Guardar snapshot de inventario al cierre de caja
        try:
            from app.services.inventario_service import SnapshotInventarioService
            snapshot_svc = SnapshotInventarioService(self._db, self._auth)
            snapshot_svc.guardar_snapshot(
                caja_id,
                nota=f"Snapshot al cierre de caja. Diferencia: {diferencia}"
            )
        except Exception as e:
            # Log pero no fallar el cierre si el snapshot no se guarda
