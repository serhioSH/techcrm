...
class VentaService(ServicioProtegido):
    ...
    # ---- Cobro ----
    def cobrar(self, venta_id: int, metodo_pago: str, efectivo_pago: float = 0, transferencia_pago: float = 0) -> bool:
        """
        Cierra la venta. Retorna False si ya fue cobrada (proteccion doble cobro).
        """
        self._requerir(Permisos.COBRAR)
        row = self._db.fetchone(
            "SELECT cobro_procesado, estado, total FROM ventas WHERE id=?;", (venta_id,)
        )
        if not row or row["cobro_procesado"] == 1 or row["estado"] != "ABIERTA":
            return False
        
        # Si no se especificó efectivo_pago/transferencia_pago, usar metodo_pago
        if metodo_pago == "EFECTIVO" and efectivo_pago == 0:
            efectivo_pago = row["total"]
            transferencia_pago = 0
        elif metodo_pago == "TRANSFERENCIA" and transferencia_pago == 0:
            efectivo_pago = 0
            transferencia_pago = row["total"]
        
        self._db.execute(
            """UPDATE ventas
               SET estado='CERRADA', metodo_pago=?, efectivo_pago=?, transferencia_pago=?,
                   cobro_procesado=1, fecha_hora=datetime('now','localtime')
               WHERE id=?;""",
            (metodo_pago, efectivo_pago, transferencia_pago, venta_id),
        )
        self._db.commit()
        return True
    ...
    def eliminar_comprobante(self, venta_id: int, nota: str = "") -> bool:
        """
        Elimina una venta completamente de la BD sin restricciones.
        Elimina la venta y todos sus detalles asociados.
        ...
        # Eliminar sin restricciones: desactivar triggers temporalmente
        try:
            with self._db.transaccion():
                # Desactivar triggers de protección de historial
                self._db.execute("DROP TRIGGER IF EXISTS trg_ventas_no_eliminar;")
                self._db.execute("DROP TRIGGER IF EXISTS trg_detalle_no_eliminar_cerrada;")
                ...
                # Recrear los triggers (se recrearán cuando se reinicie la app)
                # o no hacerlo para permitir eliminaciones futuras
            return True
        except Exception as e:
            self._db.rollback()
            raise ValueError(f"No se pudo eliminar la venta: {e}")
